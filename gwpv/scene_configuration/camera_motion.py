...
def make_camera_swing_keyframes(
    shot_config, start_time, end_time, prev_shot_config=None
):
    num_path_points = 1
    for key in ["Position", "FocalPoint"]:
        if isinstance(shot_config[key][0], list):
            if num_path_points == 1:
                ...
                assert len(shot_config[key]) == num_path_points, (
                    "Camera shots must have either one, or the same number of"
                    " points in 'Position' and 'FocalPoint'"
                )
    position_path_points = get_flat_camera_swing_path_points(
        shot_config, "Position", num_path_points
    )
    focal_path_points = get_flat_camera_swing_path_points(
        shot_config, "FocalPoint", num_path_points
    )
    if prev_shot_config is not None:
        position_path_points = (
            get_last_camera_swing_path_point(prev_shot_config, "Position")
            + position_path_points
        )
        focal_path_points = (
            get_last_camera_swing_path_point(prev_shot_config, "FocalPoint")
            + focal_path_points
        )
        assert prev_shot_config["ViewAngle"] == shot_config["ViewAngle"], (
            "The 'Path-based' camera interpolation mode doesn't support"
            " interpolating the ViewAngle"
        )
...
def apply_swings(
    ...
    for i, shot in enumerate(camera_swings_config):
        if i > 0:
            assert "Time" in shot and "SwingDuration" in shot, (
                f"Set the 'Time' when camera shot {i} should be shown in scene"
                " time and the 'SwingDuration' to the shot in real-time"
                " seconds."
            )
            if shot["Time"] <= scene_time_range[0]:
                logger.debug(
                    f"Discarding unused camera shot {i - 1} that finishes"
            ...
            if (
                ...
                logger.debug(
                    f"Discarding unused camera shots {i} and later that are"
                    " shown after animation ends."
                )
                i_shots_end = i
                break
    ...
    camera_key_frames = []
    prev_shot_start_time = 0.0
    for i, shot in enumerate(camera_swings_config):
        shot_start_time = (
            shot["Time"] if "Time" in shot else scene_time_range[0]
        )
        shot_start_time_normalized = normalized_time_from_scene(shot_start_time)
        swing_start_time = max(
            prev_shot_start_time,
            shot_start_time
            - (
                get_scene_time(shot["SwingDuration"], scene_time_from_real)
                if "SwingDuration" in shot
                else 0.0
            ),
        )
        ...
        if i > 0:
            logger.debug(
                f"Camera shot {i} starts swinging at"
                f" {swing_start_time_normalized}."
            )
            prev_shot = camera_swings_config[i - 1]
        else:
            prev_shot = None
        logger.debug(
            f"Camera shot {i} is shown at {shot_start_time_normalized}."
        )
        camera_key_frames += make_camera_swing_keyframes(
            shot,
            swing_start_time_normalized,
            shot_start_time_normalized,
            prev_shot,
        )
